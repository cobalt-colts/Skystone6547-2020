package org.firstinspires.ftc.teamcode.oldPrograms.usedAtState.stateAutons;
...
import org.firstinspires.ftc.teamcode.oldPrograms.usedAtState.stateActions.MoveGrabberSlideTime;
import org.firstinspires.ftc.teamcode.oldPrograms.usedAtState.stateActions.Outtake;
...
@Disabled
public class DoubleSkystoneFoundationState extends LinearOpMode {
    ...
    public void runOpMode()
    {
        ...
        if (bot.isSkystone(bot.colorSensorSideLeft))
        {
            ...
            if (!bot.isStoneAtIntake()) bot.driveForward(25);
            bot.driveBackward(12);
            //bot.followTrajectorySync(bot.trajectoryBuilder()
...
//        if (isRed) bot.turnRealtiveSync(Math.toRadians(300));
       ...
       // bot.moveGrabberSlideForTime(1,1000);
        ...
        //bot.RetractGrabberSlide();
        bot.moveGrabberSlideForTime(-1,2000);

        //turn foundation toward wall

        if (isRed) bot.turnSync(Math.toRadians(-200));
        else bot.turnSync(Math.toRadians(290));
        ...
        if (isRed) bot.driveBackward(6);
        else bot.driveBackward(10);
