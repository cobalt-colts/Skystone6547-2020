...
package org.firstinspires.ftc.teamcode.util.pathfinding.aStar;

import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;

import org.firstinspires.ftc.teamcode.util.pathfinding.Rectangle;
...
public class Map<T extends AbstractNode> {
    ...
    List<Rectangle> fieldParts = new ArrayList<Rectangle>();
    ...
    public Map(int width, int higth, NodeFactory nodeFactory) 
    {
        ...
        this.width = width - 1;
        this.higth = higth - 1;
        this.ogHigth = higth-1;
        this.ogWidth = width-1;
        initEmptyNodes();
    }
    ...
    public final List<T> findPath(int oldX, int oldY, int newX, int newY) {
    	return findPath(oldX, oldY, newX, newY, true);
    }
    public final List<T> findPath(int oldX, int oldY, int newX, int newY, boolean scaleNums) {
    	if (scaleNums)
    	{
    		oldX*=scale;
    		oldY*=scale;
     ...
     *  ADDED BY DREW DANDA
     */
    public void fillRectangle(Rectangle rectangle) {fillRectangle(rectangle,true);}
    public void fillRectangle(Rectangle rectangle,boolean addToList)
    {
    	int x1 = (int) (rectangle.getX1() * scale);
    	int x2 = (int) (rectangle.getX2() * scale);
    	int y1 = (int) (rectangle.getY1() * scale);
    	int y2 = (int) (rectangle.getY2() * scale);
    	...
    	for (int i = 0; i < xRange;i++)
    	{
    		...
    		{
    			setWalkable(i+x1, j+y1, false);
    		}
    	}
    	
    	if (addToList)
    	{
    	//add rect to list of rects if it hasn't been added already
    		try {
    		
    			for (Rectangle thing : fieldParts)
    			{
    				if (thing.equals(rectangle)) return; 
    			}
    			System.out.println("added rect");
    			fieldParts.add(rectangle);
    		} catch (Exception e) {
    ...
    public void fillRectangles(List<Rectangle> rectangles)
    {
    	...
    	{
    		fillRectangle(rectangle, false);
    	}
    }
    public void setScale(double scale)
    {
    	this.scale = scale;
    	this.width = (int) ((ogWidth+1) * scale)-((int) (1+scale));
    	this.higth = (int) ((ogHigth+1) * scale)-((int) (1+scale));
    	
    	initEmptyNodes();
    	fillRectangles(fieldParts);
    }
    public void printPath(List<ExampleNode> path)
    {
...
//        }
    	...
    	for (int i = 0; i < path.size(); i++) {
    		setWalkable(path.get(i).getxPosition(), path.get(i).getyPosition(), false);
    	}
    	drawMap();
    	System.out.println("printed map");
//    	for (int i = 0; i < path.size(); i++)
//    	{
//    		setWalkable(path.get(i).getxPosition(), path.get(i).getyPosition(), true);
//    	}
    }
